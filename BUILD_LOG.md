...
3. **Live replay of one fraud case (1:45–2:45).** Pull the exact `payment_risk_check_failed` record from `audit_log.jsonl` and read the gate's decision aloud — this is the rubric's "one failure handled gracefully," happening on camera from a real log line, not staged.
...
- **Test coverage gap.** Only `gate.py` and `decline_codes.py` have dedicated unit tests. `ollama_client.py`'s retry/backoff path, `generate_data.py`'s distribution logic, and the checkpoint/resume logic are validated empirically (the real runs) but not by repeatable tests.
- **The official-MCP-server path is implemented and probed (tool schemas verified live), but not yet exercised end-to-end with real keys** — no Razorpay account has been created yet. The simulate-mode path is the only one that has actually processed all 150 records so far.
...
- **A resolved risk, kept here for the record:** the schema-clarity bug (§3.6, §9) shows the 46% post-fix override rate is still the *honest current number*, not a floor — a larger or better-tuned model would likely lower it further; this project deliberately didn't chase that, since the point being demonstrated is the gate's necessity, not the model's ceiling.
