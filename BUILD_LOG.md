...
- **The 150-record LLM-accuracy figures are really 15 unique test cases, found and then tested for real.** `decline_description` is a fixed string per decline code, `temperature: 0` makes output deterministic, and no rule references amount or record ID — so every record sharing a code gets the exact same proposed action every time (verified directly against the audit log: zero variance within any code, across all four runs). The 150 number is real and meaningful for the revenue metrics, but overstates test diversity for the LLM-accuracy number specifically. Tested this directly rather than just disclosing it: ran 16 hand-written alternate phrasings of the same 16 situations, sharing no wording with the fixed catalog — **16/16 (100%) matched policy**, real evidence the fix generalizes to *how* a situation is described, not just the exact strings it was tuned against. Still doesn't prove robustness to decline codes outside this catalog, ambiguous/multi-reason real-world text, or adversarial phrasing — none of that has been built or tested. Full detail: `METRICS.md` §2.4.
