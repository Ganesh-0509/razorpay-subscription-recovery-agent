...
- **Test coverage gap, partially closed.** `gate.py`, `decline_codes.py`, and now `ollama_client.py`'s no-tool-call/malformed-arguments/retry-exhaustion paths (D4, D8 in §7.3) all have dedicated, mocked unit tests — 43 total project-wide (grew from 30 with the compliant-escalation rule tests and the `agent.py`/`agent_onetime.py` refactor's regression test below), no live Ollama server needed, so they run in CI. Still not covered by a repeatable test: `generate_data.py`'s distribution logic and the checkpoint/resume logic (D9) — both validated empirically by the real runs, not by a test.
